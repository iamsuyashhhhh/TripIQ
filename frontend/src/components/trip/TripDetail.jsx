export default function TripDetail({ trip, onGenerate, expense, setExpense, onAddExpense }) {
  return (
    <section className="panel detail-panel">
      <div className="detail-header">
        <div>
          ...
          <h2>{trip.title}</h2>
          <p>
            {trip.destination}, {trip.country} · ${trip.budget} budget · {trip.travelers} travelers
          </p>
        </div>
        <button className="primary compact" onClick={onGenerate}>
          Generate itinerary ↗
        </button>
      </div>
      {trip.itinerary?.days?.length ? (
        ...
        <div className="empty">No itinerary yet. Generate one from your trip preferences.</div>
      )}
      <div className="expense-area">
        ...
        <form className="expense-form" onSubmit={onAddExpense}>
          ...
          <input
            ...
            onChange={(event) => setExpense({ ...expense, estimated_amount: event.target.value })}
          />
        ...
        {trip.expenses?.map((item) => (
          <div className="expense-item" key={item.id}>
            ...
            <strong>${item.estimated_amount}</strong>
          </div>
        ))}
      </div>
    </section>
